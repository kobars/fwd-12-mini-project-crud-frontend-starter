...
import ApiInspector from './components/ApiInspector.jsx'
...
export default function App() {
  ...
  function actions(item) {
    return <div className="card-actions"><button onClick={() => openItem(item.id, 'edit')} aria-label={'Edit ' + item.title}>Edit {config.singular}</button><button className="delete" onClick={() => openItem(item.id, 'delete')} aria-label={'Hapus ' + item.title}>Hapus</button></div>
  }
  ...
  return <>
    ...
    <main className="shell app-main" id="main">
      ...
      <div className="page-heading"><div><p className="eyebrow">{config.name.toUpperCase()}</p><h1>{title}</h1><p className="intro-text">{description}</p></div><button className="button primary" disabled={state.loading || !!state.error} onClick={() => create(route.type === 'categories' ? 'category' : 'item')}>+ Tambah {route.type === 'categories' ? 'kategori' : config.singular}</button></div>
      ...
      <div className="collection-bar"><p role="status">{state.loading ? 'Memuat data...' : state.error ? 'Data belum tersedia' : route.type === 'categories' ? state.categories.length + ' kategori' : state.items.length + ' ' + config.singular}</p><span>{route.type === 'categories' ? 'Buka kategori untuk melihat data terkait.' : actor ? 'Uji CRUD sebagai ' + actor.name + '. Hak akses diperiksa backend.' : 'Uji sebagai pengunjung: perubahan produk/kursus akan ditolak backend.'}</span></div>
      ...
      <ApiInspector actor={actor} />
    </main>
    <footer className="footer"><div className="shell footer-inner"><a href="#catalog" className="wordmark small">FWD 12 LMS</a><p>{config.name}</p><a href="#catalog" className="text-link">Kembali ke katalog</a></div></footer>
